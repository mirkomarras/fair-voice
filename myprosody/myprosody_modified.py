...
def myprosody(m,p):
    sound = m
    sourcerun = p if os.path.splitext(p)[1] == '.praat' else os.path.join(p, "essen", "MLTRNL.praat")
    path = os.path.dirname(m)
    myprosody_path = os.path.dirname(os.path.dirname(p)) if os.path.splitext(p)[1] == '.praat' else p
    outo=os.path.join(myprosody_path, "datanewchi22.csv")
    outst=os.path.join(myprosody_path, "datanewchi44.csv")
    outsy=os.path.join(myprosody_path, "datanewchi33.csv")
    pa2=os.path.join(myprosody_path, "stats.csv")
    pa7=os.path.join(myprosody_path, "datanewchi44.csv")
...
def mysplev(m, p):
    ...
    path = os.path.dirname(m)
    myprosody_path = os.path.dirname(os.path.dirname(p)) if os.path.splitext(p)[1] == '.praat' else p
    pa1 = os.path.join(myprosody_path, "datanewchi23.csv")
    pa7 = os.path.join(myprosody_path, "datanewchi45.csv")
    pa5 = os.path.join(myprosody_path, "datanewchi34.csv")
    result_array = np.empty((0, 100))
    ph = sound
    files = glob.glob(ph)
    result_array = np.empty((0, 27))
    try:
        ...
        namess = ['avepauseduratin', 'avelongpause', 'speakingtot', 'articulationrate', 'mr',
                        'q50', 'std', 'fmax', 'fmin', 'vowelinx2', 'formantmean', 'formantstd', 'ins',
                        'xxx']
        df1 = pd.read_csv(pa5,
                          names=namess)
        ...
        bg = p
        bi = myspp(bp, bg)
        result_dict = {"data": x, "bi": bi}
        if bi < 85:
            print("Try again, unnatural-sounding speech detected. No further result")
            return None

        filename = os.path.join(myprosody_path, "essen", "CART_model.sav")
        model = pickle.load(open(filename, 'rb'))
        predictions = model.predict(x)
        print("58% accuracy    ", predictions)
        result_dict['CART_model'] = predictions

        filename = os.path.join(myprosody_path, "essen", "KNN_model.sav")
        model = pickle.load(open(filename, 'rb'))
        predictions = model.predict(x)
        print("65% accuracy    ", predictions)
        result_dict['KNN_model'] = predictions

        filename = os.path.join(myprosody_path, "essen", "LDA_model.sav")
        model = pickle.load(open(filename, 'rb'))
        predictions = model.predict(x)
        print("70% accuracy    ", predictions)
        result_dict['LDA_model'] = predictions

        filename = os.path.join(myprosody_path, "essen", "LR_model.sav")
        model = pickle.load(open(filename, 'rb'))
        predictions = model.predict(x)
        print("67% accuracy    ", predictions)
        result_dict['LR_model'] = predictions

        filename = os.path.join(myprosody_path, "essen", "NB_model.sav")
        model = pickle.load(open(filename, 'rb'))
        predictions = model.predict(x)
        print("64% accuracy    ", predictions)
        result_dict['NB_model'] = predictions

        filename = os.path.join(myprosody_path, "essen", "SVN_model.sav")
        model = pickle.load(open(filename, 'rb'))
        predictions = model.predict(x)
        print("63% accuracy    ", predictions)
        result_dict['SVN_model'] = predictions
    except:
        print("Try again the sound of the audio was not clear")