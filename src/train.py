...
def main():
    ...
    if args.from_csv == 0:
        ...
        val_data = load_val_data(args.val_base_path, args.val_pair_path, args.val_n_pair, args.sample_rate,
                                 args.n_seconds)
    else:
        x_train, y_train = load_data_from_csv(args.train_csv_path, args.audio_dir)
        x_test = []

    metadata = pd.read_csv(os.path.join(args.audio_dir, 'metadata.csv'))

    classes = len(np.unique(y_train))

    g = np.array([1 if metadata[(metadata["id_user"] == x.split(os.sep)[5]) &
                          (metadata["language_l1"] == x.split(os.sep)[4])]["gender"].to_numpy()[0] == "male" else 0
         for x in x_train])

    """
    mapping = np.zeros(classes)

    #print(classes)
    for x, label in zip(x_train, y_train):
        mapping[label] = True if metadata[(metadata["id_user"] == x.split(os.sep)[5]) &
                                          (metadata["language_l1"] == x.split(os.sep)[4])]["gender"].to_numpy()[0] == "male" else False
    """
    ...
    try:
        ...
        for index, x in enumerate(
                data_pipeline_generator_verifier(x_train[:0], y_train[:0], classes, sample_rate=args.sample_rate,
    ...
    train_data = data_pipeline_verifier(x_train, y_train, g, classes, sample_rate=args.sample_rate,
                                        n_seconds=args.n_seconds, batch=args.batch, dim=dim, input_format=input_format,
                                        num_fft=num_fft, spec_len=spec_len)
